<template>
  <div class="w-full bg-white rounded-2xl border border-gray-200/90 p-4 sm:p-5 shadow-2xs">
    <!-- Header: Pill Badge Title + Controls -->
    <div class="flex items-center justify-between gap-3 pb-3.5 border-b border-gray-100">
      <div class="flex items-center gap-2.5">
        <div class="flex items-center gap-1.5 bg-red-50 text-[#E31B23] border border-red-200/70 px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
          <span class="w-2 h-2 rounded-full bg-[#E31B23] animate-pulse" />
          <span>Live Shopping &amp; Reels</span>
        </div>
        <h3 class="text-sm sm:text-base font-outfit font-black text-gray-900 tracking-tight hidden sm:inline">
          Tonton &amp; Belanja Interaktif
        </h3>
      </div>
    ...
    <!-- Horizontal Scroll Track with Real Reels / Story Cards -->
    <div
      ref="scrollRef"
    ...
    >
      ...
      <template v-if="loading">
        <div
          v-for="n in 5"
          :key="n"
          class="w-[155px] sm:w-[170px] aspect-[9/14] shrink-0 rounded-2xl bg-gray-200 animate-shimmer"
        />
      </template>

      <!-- Real Cards (Directly derived from real API video sessions) -->
      <template v-else>
        <div
          v-for="card in displayCards"
          :key="card.id"
          class="group relative w-[155px] sm:w-[170px] aspect-[9/14] shrink-0 rounded-2xl overflow-hidden cursor-pointer shadow-xs hover:shadow-md transition-all duration-300 bg-neutral-900"
        ...
        >
          ...
          <!-- Gradient Ambient Shade -->
          <div class="absolute inset-0 bg-gradient-to-t from-black/85 via-black/25 to-black/30 pointer-events-none" />

          <!-- Top Status Bar: Live / Video + Viewers / Likes -->
          <div class="absolute top-2.5 left-2.5 right-2.5 flex items-center justify-between z-10">
            <span
              v-if="card.is_live"
              class="flex items-center gap-1 bg-[#E31B23] text-white text-[9.5px] font-black px-2 py-0.5 rounded-full shadow-xs uppercase tracking-wide"
            >
            ...
            <span
              v-else
              class="flex items-center gap-1 bg-black/60 backdrop-blur-xs text-white text-[9.5px] font-bold px-2 py-0.5 rounded-full shadow-xs uppercase"
            >
            ...
            <!-- Viewers / Likes Pill -->
            <div class="flex items-center gap-1 bg-black/50 backdrop-blur-xs text-white text-[10px] font-medium px-2 py-0.5 rounded-full border border-white/10">
              <Eye v-if="card.is_live" class="w-3 h-3 text-white/80" />
              <Heart v-else class="w-3 h-3 text-white/80" />
              <span>{{ card.is_live ? (card.viewers || 180) : (card.likes || 120) }}</span>
            </div>
          </div>

          <!-- Play Center Overlay (Appears on Hover) -->
          <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10">
            <div class="w-10 h-10 rounded-full bg-[#E31B23]/90 text-white flex items-center justify-center shadow-lg transform group-hover:scale-110 transition-transform">
              <Play class="w-5 h-5 fill-white ml-0.5" />
            </div>
          </div>

          <!-- Bottom Metadata: User Avatar + Handle + Caption -->
          <div class="absolute bottom-2.5 left-2.5 right-2.5 text-white z-10">
            <div class="flex items-center gap-1.5 mb-1">
              <div class="w-5 h-5 rounded-full overflow-hidden border border-white/60 bg-white/20 shrink-0">
                <img :src="card.user_avatar || card.thumbnail" :alt="card.user_name" class="w-full h-full object-cover" />
              </div>
              <span class="text-[11px] font-bold text-white truncate drop-shadow-xs">
                @{{ card.user_name }}
              </span>
            </div>
            <p class="text-[10.5px] font-medium text-gray-200 line-clamp-2 leading-tight drop-shadow-xs">
    ...
    <!-- Video Player Modal for Interactive Reel Playback -->
    <div
      v-if="selectedVideo"
    ...
    >
      <div class="relative w-full max-w-sm aspect-[9/16] rounded-3xl overflow-hidden bg-black shadow-2xl flex flex-col justify-between">
        ...
        <!-- Host Info Overlay inside Video Modal -->
...
// Fallback verified MongoDB items if props.sessions is not yet populated
