<template>
  <div class="w-full bg-white rounded-2xl border border-gray-200/90 p-4 sm:p-5 shadow-2xs">
    <!-- Header: Deal-Centric Shoppable Live -->
    <div class="flex items-center justify-between gap-3 pb-3 border-b border-gray-100">
      <div class="flex items-center gap-2">
        <h3 class="font-outfit font-black text-gray-900 text-sm sm:text-base tracking-tight uppercase">
          BEKASKU LIVE &amp; SHOPPABLE VIDEO
        </h3>
        <span class="bg-[#EBAD25] text-gray-900 text-[10px] font-black px-2 py-0.5 rounded-full uppercase">
          Live Sale
        </span>
      </div>
    ...
    <!-- Scroll Track (Shoppable Live Stream Style using Real Data) -->
    <div
      ref="scrollRef"
    ...
    >
      <template v-if="loading">
        <div
          v-for="n in 4"
          :key="n"
          class="w-[175px] sm:w-[190px] aspect-[9/14] shrink-0 rounded-2xl bg-gray-200 animate-shimmer"
        />
      </template>
      ...
      <template v-else>
        <div
          ...
          :key="card.id"
          class="group relative w-[175px] sm:w-[190px] aspect-[9/14] shrink-0 rounded-2xl overflow-hidden cursor-pointer shadow-xs hover:shadow-md transition-all duration-300 bg-neutral-900"
        ...
        >
          ...
          <img
            ...
            loading="lazy"
          />
          <div class="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-black/30 pointer-events-none" />

          <!-- Top Status -->
          <div class="absolute top-2.5 left-2.5 right-2.5 flex items-center justify-between z-10">
            <span
              v-if="card.is_live"
              class="flex items-center gap-1 bg-[#E31B23] text-white text-[9px] font-black px-2 py-0.5 rounded-full uppercase shadow-xs"
            >
            ...
            <span
              v-else
              class="flex items-center gap-1 bg-black/60 backdrop-blur-xs text-white text-[9px] font-bold px-2 py-0.5 rounded-full uppercase shadow-xs"
            >
              <Video class="w-3 h-3 text-white" />
              <span>VIDEO</span>
            </span>

            <span class="text-[10px] text-white font-medium bg-black/50 backdrop-blur-xs px-2 py-0.5 rounded-full border border-white/10">
              {{ card.is_live ? `${card.viewers || 180} menonton` : `${card.likes || 120} suka` }}
            </span>
          </div>

          <!-- Bottom: Floating Product Deal Bubble (Shoppable Stream Feature with Real Data) -->
          <div class="absolute bottom-2 left-2 right-2 space-y-1.5 z-10">
            <!-- Shoppable Product Micro-Card -->
            <div class="bg-white/95 backdrop-blur-md p-1.5 rounded-xl border border-white/60 shadow-lg flex items-center gap-2">
              <img
                :src="card.thumbnail || card.user_avatar"
                :alt="card.title"
              ...
              <div class="min-w-0 flex-1">
                <p class="text-[10px] font-bold text-gray-900 truncate">
                  {{ card.title || 'Produk Preloved' }}
                </p>
                <p class="text-[11px] font-black text-[#2563EB] leading-none">
                  {{ getPriceEstimate(card) }}
                </p>
              </div>
              <ShoppingBag class="w-4 h-4 text-[#2563EB] shrink-0 mr-1" />
            </div>

            <!-- Presenter Name -->
            <div class="flex items-center gap-1 text-[10px] text-gray-300 px-1">
              <span class="font-bold text-white truncate">@{{ card.user_name }}</span>
            </div>
          </div>
        </div>
      </template>
...
import { ChevronLeft, ChevronRight, ShoppingBag, Video, X } from 'lucide-vue-next'
...
function getPriceEstimate(card: VideoFeedItem): string {
  if (card.title?.includes('85.000')) return 'Rp 85.000'
  if (card.title?.includes('150.000')) return 'Rp 150.000'
  if (card.title?.includes('Senapan')) return 'Rp 450.000'
  return 'Mulai Rp 65.000'
}
